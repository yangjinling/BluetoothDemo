...
public class BluetoothChatUtil {
    ...
    public static final int ACCEPT_SUCCESS = 9;
    ...
    private void connectionSuucced() {// accept成功
        Message msg = mHandler.obtainMessage(ACCEPT_SUCCESS);
        mHandler.sendMessage(msg);
    }
    ...
    private class AcceptThread extends Thread {
        ...
        public void cancel() {
            ...
            try {
                ...
                Log.e(TAG, "close() of activity_server_bluetooth failed", e);
            }
        }
    }
            ...
            while (true) {
                try {
                    if ((bytes = mmInStream.read(buffer)) > 0) {
                        ...
                        Log.e("YJL", "ConnectedThread************数据返回为：：：" + strReadDate);
                        Message msg = mHandler.obtainMessage(MESSAGE_READ);
                        Bundle bundle = new Bundle();
                        bundle.putString(READ_MSG, strReadDate);
                        msg.setData(bundle);
                        mHandler.sendMessage(msg);
                    }
                } catch (IOException e) {
